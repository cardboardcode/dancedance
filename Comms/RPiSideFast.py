...
#                                       print(", ".join(sample))
                                        self.energy = self.energy + (0.004*sample[0]*sample[1])
                                        self.totalTimeElapsed = self.totalTimeElapsed+0.004
                                        self.CumuPower = (self.energy / self.totalTimeElapsed)
                                        sample = sample + [self.CumuPower]
...
class Raspberry():
        def __init__(self):
                ...
                self.threads = []
                self.headers = [' Vol  ', ' Cur  ', '  X1  ', '  Y1  ', '  Z1  ', '  X2  ', '  Y2  ', '  Z2  ', '  X3  ', '  Y3  ', '  Z3  ', '  X4  ', '  Y4  ', '  Z4  ','CumuPower']
                self.time0 = [0]
                self.predAction = 'frontBack' #dummy software side must put in the data. 
        ...
        def main(self):
                try:
                        ...
                        #MLThread = (); Get the predicted action and assign it to self.predAction
                        clientThread = finalClientImproved(self.predAction, self.list[1], self.list[2], power = self.list[1]*self.list[2], self.list[16], socket[0], socket[1])
